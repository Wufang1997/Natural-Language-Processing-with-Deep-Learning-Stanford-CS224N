...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        """the dimension is self.embed_size * self.n_features instead of only self.embed_size """
        self.embed_to_hidden = nn.Linear(self.embed_size * self.n_features, self.hidden_size)
        """ the nn.init.xavier_uniform_ is used to initialize the weight"""
        nn.init.xavier_uniform_(self.embed_to_hidden.weight, gain=1.0)
        self.dropout = nn.Dropout(p = self.dropout_prob)
        """ the dimension now is only self.hidden_size """
    ...
    def embedding_lookup(self, t):
        ...
        x = embeddings.view(embeddings.shape[0], embeddings.shape[1] * embeddings.shape[2])
        ### TODO:
    ...
    def forward(self, t):
        ...
        hidden = self.dropout(nn.functional.relu_(self.embed_to_hidden(embeddings)))
