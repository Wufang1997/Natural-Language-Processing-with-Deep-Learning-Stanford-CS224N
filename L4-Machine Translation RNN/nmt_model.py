...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(embed_size + hidden_size, hidden_size, bias=True) # note it's (e+h) input size
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1, dim=0):   # dim = 1, not len(self.vocab.tgt) (why? hh)
            Y_t = torch.squeeze(Y_t)   # shape (b, e)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        (dec_hidden, dec_cell) = dec_state   # it's a much easier way
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(2)).squeeze(2)  # e_t shape (b, src_len)
        ...
        ###         - enc_hiddens_proj is shape (b, src_len, h) andcorresponds to W_{attProj} h^enc (batched).
        ...
        """for the softmax with dim=0, the sum of each column =1, while for dim=1, it is the sum of the rows
         that equals 1. Usually, you do not want to perform a softmax operation across the batch dimension."""
        # a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)
        att_view = (alpha_t.shape[0], 1, alpha_t.shape[1])
        a_t = torch.bmm(alpha_t.view(*att_view), enc_hiddens).squeeze(1)  # I don't know why we cannot use the above method
