...
def naiveSoftmaxLossAndGradient(
    ...
    ### YOUR CODE HERE
    value = np.exp(np.sum(outsideVectors * centerWordVec, axis = 1))
    y_hat = value / np.sum(value) # or y_hat = softmax(value)
    loss = - np.log(y_hat[outsideWordIdx])  # must put forward
    dif = y_hat; dif[outsideWordIdx] -= 1


    gradCenterVec = np.dot(dif.T, outsideVectors)
    gradOutsideVecs = np.dot(dif[:, np.newaxis], np.array([centerWordVec]))
