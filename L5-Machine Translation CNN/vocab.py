...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for lst in sents:
            char = []
            for word in lst:
                char2id = [self.start_of_word] + [self.char2id[x] for x in word] + [self.end_of_word]
                char.append(char2id)
            word_ids.append(char)
        return word_ids
        ### TODO: 
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        charids = self.words2charindices(sents)
        sents_t = pad_sents_char(charids, self['<pad>'])
        sents_var = torch.tensor(sents_t, dtype=torch.long, device=device).permute(1, 0, 2)
        return sents_var   # transpose
        ### TODO: 
